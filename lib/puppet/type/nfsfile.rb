Puppet::Type.newtype(:nfsfile) do
  ...
  newproperty(:mode) do
    desc "The desired permissions mode for the file in symbolic or numeric notiation. This value must be specified as a string; do not use un-quoted number so to represent file modes."
    newvalues(/^0\d{3}/)
  end

  newparam(:manage_as) do
    desc "The user whom Puppet should use to manage the file."
  end
end
